...
| Feature | Description |
|---------|-------------|
| Bi-temporal facts | Track when facts were true (event time) vs when we learned them (transaction time, see [Graphiti](https://github.com/getzep/graphiti)) |
| Edge invalidation | New contradicting facts invalidate old ones with temporal bounds |
| Predict-calibrate | Only extract knowledge that fails prediction (importance emerges from error, see [Nemori](https://arxiv.org/abs/2508.03341)) |
| Episode segmentation | Topic-based chunking, not arbitrary splits |
| Hybrid retrieval | Vector + BM25 + graph traversal with RRF fusion |
...
Ablation studies on memory benchmarks show that **56.7% of temporal reasoning performance** comes from disambiguating facts at write-time rather than retrieval-time (see [SimpleMem paper](https://arxiv.org/abs/2504.13716)):
...
- [x] SQLite storage
  ...
  - [x] Vector storage via [sqlite-vec](https://github.com/asg017/sqlite-vec)
  - [x] [FTS5](https://www.sqlite.org/fts5.html) for full-text
- [x] Extract protocols from implementations (`MessageStore`, `EpisodeStore`, `KnowledgeStore`, `EmbeddingClient`)
- [x] Embedding client implementations ([OpenAI](https://platform.openai.com/docs/guides/embeddings), [FastEmbed](https://github.com/qdrant/fastembed))
...
- [x] LLM client implementations ([PydanticAI](https://ai.pydantic.dev/) adapter)
...
- [ ] ~~**Atomization step**~~ — DEFERRED TO v0.1.1/v0.2.0
...
- [ ] PostgreSQL implementation
  - [pgvector](https://github.com/pgvector/pgvector) for embeddings
  - [pg_trgm](https://www.postgresql.org/docs/current/pgtrgm.html) for text search
  - Recursive CTEs for graph traversal
  - Connection pooling ([asyncpg](https://github.com/MagicStack/asyncpg))
...
- [ ] Benchmarking
  - [LongMemEval](https://github.com/xiaowu0162/LongMemEval) integration
...
| Observability | [structlog](https://www.structlog.org/) + optional OTEL | Structured logging default, tracing as extra |
